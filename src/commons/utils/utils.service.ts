...
export class UtilsService {
  ...
  public validateCapitalizeWords(request: string): string {
    if (!request) return '';
  ...
  public pagination(request: IPagination): IPagination {
    ...
    const getSort = request.sort ? (request.sort as IPagination['sort']) : undefined;
  ...
  public paginationInfiniteResponse<T>(request: IPaginationResponse<T>): IPaginationResponse<T> {
    ...
    const getMore = getTotalData > getPage * getLimit;
